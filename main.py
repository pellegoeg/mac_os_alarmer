...
def main():
    ...
    if notifier_type == "teams":
        webhook_url = os.getenv(
            "TEAMS_WEBHOOK_URL", notifier_config.get("teams_webhook_url")
        )
        if not webhook_url:
            logger.error(
                "TEAMS_WEBHOOK_URL er ikke sat,falder tilbage til macOS-notifikationer."
            )
    ...
    db_params = {
        ...
        "database": os.getenv(
            "DB_NAME", db_config.get("database", db_config.get("dbname", "postgres"))
        ),
    ...
    for check_def in checks_list:
        ...
        if check_type == "postgresql":
            check = PostgreSQLCheck(
                ...
                alert_message_template=check_def.get(
                    "alert_message",
